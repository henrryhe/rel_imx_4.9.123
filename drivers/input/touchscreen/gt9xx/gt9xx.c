...
void gtp_int_output(struct goodix_ts_data *ts, int level)
{
	...
	if (level == 0) {
		if (gpio_is_valid(ts->pdata->irq_gpio)) {
			...
			dev_err(&ts->client->dev, "Failed set int pin to output low\n");
		}
	} else {
		if (gpio_is_valid(ts->pdata->irq_gpio)) {
			gpio_direction_output(ts->pdata->irq_gpio, 1);
		} else {
			dev_err(&ts->client->dev, "Failed set int pin to output high\n");
		}
	}
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	if (gpio_is_valid(ts->pdata->rst_gpio)) {
		...
		dev_err(&ts->client->dev, "Failed set rst pin to output low\n");
	}
	...
	if (gpio_is_valid(ts->pdata->rst_gpio)) {
		...
		dev_err(&ts->client->dev, "Failed set rst pin to output high\n");
	}
	...
	if (gpio_is_valid(ts->pdata->rst_gpio)) {
		...
		dev_err(&ts->client->dev, "Failed set rst pin to input\n");
	}
...
static int gtp_pinctrl_init(struct goodix_ts_data *ts)
{
	...
	pinctrl_select_state(pinctrl->pinctrl, pinctrl->default_sta);		
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	dev_info(dev, "irq request gpio: %d, rst request gpio: %d\n", 
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < 0) {
			dev_err(&ts->client->dev, "[Esd] check func read i2c error: %d\n", i);
			continue;
		}
